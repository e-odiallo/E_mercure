...
function hanoiMain(mode, rel) {
    ...
    s += '<span style="font: 20px Arial; text-align: center; color: black;">Disks: </span>';
    ...
    s += '<button style="margin-left: 20px; font-size: 15px; color: #000aae; " class="btn"  onclick="gameNew()" >Restart</button>';
    s += '<button style="margin-left: 2px; font-size: 15px; color: #000aae; " class="btn"  onclick="logOpen()" >Log</button>';
    s += '<button style="margin-left: 2px; font-size: 15px; color: #000aae; " class="btn"  onclick="solveIt()" >Solve!</button>';
    s += '</div>';
  ...
  <textarea  id="log" style="width:400px; height:200px;"></textarea>
    ...
    s += '<div id="copyrt" style="position:absolute; left:3px; bottom:3px; font: 10px Arial; font-weight: bold; color: blue; ">&copy; 2020 MathsIsFun.com  v' + version + '</div>';
    s += '</div>';
    document.write(s);
...
function hitTest(shape, mx, my) {
    ...
    if (my < shape.y)
        return false;
    if (mx > (shape.x + shape.wd))
        return false;
    if (my > (shape.y + shape.ht))
        return false;
    return true;
}
...
function solveAnim() {
    ...
    if (my.moveNo < my.moves.length)
        requestAnimationFrame(solveAnim);
}
...
function Pop(id, yesStr, yesFunc, noStr, noFunc) {
    this.id = id;
...
Pop.prototype.bodySet = function(s) {
